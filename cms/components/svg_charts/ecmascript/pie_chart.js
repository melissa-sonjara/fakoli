class PieChart extends Chart
{
	...
	drawChart()
	{
		...
		var total = this.values.reduce(function(a, b) { return a + b; }, 0);
		...
		this.values.forEach(function(val, idx)
		{
			...
			if (this.options.animate && val != 0)
			{
				...
				if (this._events && this._events['sectorClick'])
				{
		...
		{
			//this.sectors.forEach(function(e) {e.emboss(); });
		}

		if (this.options.shadow)
		{
			this.sectors.forEach(function(e) { e.dropShadow(5, 1, 1, 0.2); });
		}
    ...
    sector(startAngle, endAngle, index)
	{
		var cx = this.options.cx;
		var cy = this.options.cy;
		var r  = this.options.radius;
        ...
        var sectorPath = this.path(['M', cx, cy], ['L', x1, y1], ['A', r, r, 0, +(endAngle - startAngle > Math.PI), 0, x2, y2], 'z');
        return this.paper.path(sectorPath).attr(params);
	}

	label(angle, text, idx)
	{
		var cx = this.options.cx;
		var cy = this.options.cy;
		var r  = this.options.radius;
	...
	selectSectors(indices)
	{
		indices.forEach(function(idx) { this.addSectorClass(idx, 'selected');}.bind(this));
		this.showSelected();
	}
