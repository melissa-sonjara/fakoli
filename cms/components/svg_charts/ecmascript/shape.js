class Shape
{
	...
	attr()
	{
		...
		else if (arguments.length == 2)
		{
			...
			var p = this.primitives[name];
			p.attr(attrs);
		}
		return this;
	}

	animate(params, length, transition, callback)
	{
		this.shapeSet.animate(params, length, transition, callback);
		return this;
	}

	animatePrimitive(name, params, length, transition, callback)
	{
		this.primitives[name].animate(params, length, transition, callback);
		return this;
	}

	show(length, transition)
	{
		if (this.options.shown) return;

		this.options.shown = true;

		Object.values(this.primitives).forEach(function(elt)
		{
			elt.show();
		}.bind(this));
	...
	hide(length, transition)
	{
		...
		if (length)
		{
			...
			{
				Object.values(this.primitives).forEach(function(elt) { elt.hide();/* elt.unclick(this.clickHandler); */});
			}.bind(this));
		}
		else
		{
	...
	getBBox()
	{
		...
		Object.values(this.primitives).forEach(function(p)
		{
			var bb2 = p.getBBox();
		...
		var dx = this.origin.x - bb.x;
		var dy = this.origin.y - bb.y;
		bb.x = this.origin.x;
		bb.y = this.origin.y;
		bb.x2 += dx;
		bb.y2 += dy;
...
{
	constructor(paper, x, y, w, h, text, radius, options)
	{
		super(paper, options);
...
{
	constructor(paper, from, to, options, attrs)
	{
		super(paper, options);
		this.from = from;
		this.to = to;
